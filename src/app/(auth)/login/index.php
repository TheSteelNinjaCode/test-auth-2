...
if ($stateManager->getState('login')) {
    $message = $stateManager->getState('login')['message'];
    $email = $stateManager->getState('post')['email'];
    $password = $stateManager->getState('post')['password'];
}

$stateManager->resetState(['login', 'post'], true);
